...
const AuctionDetailsModal = ({ auction, onClose, web3, account }) => {
  ...
  const getBidHistory = async () => {
    ...
    try {
      const auctionContract = new web3.eth.Contract(AuctionABI.abi, auction);
      const bidder = account;

      const { amounts, timestamps } = await auctionContract.methods.getBidHistory(bidder).call();

      console.log("Bid Amounts:", amounts);
      console.log("Timestamps:", timestamps);


      alert("Bid History:\n" + JSON.stringify({ amounts, timestamps }));
    } catch (error) {
      console.error("Error fetching bid history:", error);
      alert("Failed to fetch bid history. See console for details.");
    }
  };
    ...
    <div className="auction-details-modal">
      ...
      {timeLeft !== "Auction ended" && (
        <div className="bid-section">
          <input type="number" className="bid-input" value={bidAmount} onChange={(e) => setBidAmount(e.target.value)} placeholder="Your Bid" />
      ...
      {/* istorija licitacija */}
      <button className="action-button" onClick={getBidHistory}>Get Bid History</button>
